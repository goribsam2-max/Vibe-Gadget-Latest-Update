...
const NewPassword: React.FC = () => {
  ...
  if (!email) return <div className="min-h-screen flex items-center justify-center bg-zinc-50"><Icon name="spinner" className="animate-spin text-4xl text-emerald-600" /></div>;

  return (
    <div className="p-6 md:p-12 pb-24 animate-fade-in bg-white max-w-xl mx-auto min-h-screen font-inter">
       <SEO title="Reset Password" description="Create a new password for your VibeGadget account." />
       <div className="flex items-center space-x-6 mb-12">
          <button onClick={() => navigate('/signin')} className="p-3.5 bg-zinc-50 rounded-2xl hover:bg-black hover:text-white transition-all shadow-sm">
       ...
       <div className="flex-1">
          <p className="text-zinc-600 text-sm mb-10 leading-relaxed font-medium">Create a new password for <strong className="text-black">{email}</strong>. It must be different from previously used passwords.</p>
          
          <div className="space-y-6">
             <div>
                <label className="block text-xs font-bold uppercase tracking-widest text-zinc-500 mb-2">New Password</label>
                <input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="••••••••••••" className="w-full bg-zinc-50 border border-zinc-200 rounded-2xl p-4 font-bold outline-none focus:border-black transition-colors" />
             </div>
             <div>
                <label className="block text-xs font-bold uppercase tracking-widest text-zinc-500 mb-2">Confirm Password</label>
                <input type="password" value={confirm} onChange={e => setConfirm(e.target.value)} placeholder="••••••••••••" className="w-full bg-zinc-50 border border-zinc-200 rounded-2xl p-4 font-bold outline-none focus:border-black transition-colors" />
             </div>
          </div>
       </div>
